...
{
    // Пытаемся получить строку подключения из переменных окружения (для Railway)
    ...
    {
        // Для локальной разработки (из appsettings.json)
...
using (var scope = app.Services.CreateScope())
{
    ...
    // Создаём базу данных, если её нет (НЕ УДАЛЯЕМ!)
    ...
    if (!context.Genres.Any())
    {
        ...
        context.SaveChanges();
        Console.WriteLine("Жанры добавлены");
    }
    ...
    // ==================== ДОБАВЛЯЕМ СЕРИАЛЫ (ЕСЛИ БАЗА ПУСТАЯ) ====================
    if (!context.Series.Any())
    {
        ...
        context.SaveChanges();
        Console.WriteLine($"Добавлено сериалов: {context.Series.Count()}");
    }
}
